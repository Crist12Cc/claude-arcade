...
function fallbackName(authUser: User): string {
  const meta = typeof authUser.user_metadata?.username === "string" ? authUser.user_metadata.username : "";
...
export function useSession() {
  ...
  useEffect(() => {
    ...
    const resolve = async (authUser: User | null) => {
      ...
      if (active) setUser({ name: fallbackName(authUser) });
      const { data } = await supabase
        ...
        .maybeSingle();
      if (!active) return;
      if (data?.username) setUser({ name: toArcadeName(data.username) });
    };
