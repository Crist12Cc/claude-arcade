...
export default function HallOfFamePage() {
  ...
  const tabSeed = useMemo(() => seedFromId(tab), [tab]);
  const rows = useMemo(() => seededScores(tabSeed * 23 + 7, 12), [tabSeed]);
  const game = GAMES.find((g) => g.id === tab)!;
  const youRank = user ? Math.floor(8 + (tabSeed % 4)) : null;
  const youScore = user ? Math.floor(30000 + (tabSeed % 60000)) : null;
