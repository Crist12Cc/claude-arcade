...
>
> **Nota de implementación (2026-09-13):** esta spec asumía una arquitectura (`components/games/FroggerGame.tsx` + `app/games/frogger/play/page.tsx`) que no corresponde al patrón real del repo. Se implementó siguiendo el patrón genérico ya usado por rocas/caída/bloque-buster/serpentina: `lib/games/ranaria/engine.ts` (contrato `start/destroy/setPaused/restart/onStateChange`) registrado en `components/GamePlayer.tsx`, jugado vía la ruta genérica `/jugar/ranaria`. Se reutilizó el `id` `ranaria` (mock ya sembrado en `public.games` con cover `cover-rana`, color `green`) en vez de crear una fila `frogger` nueva — no fue necesario INSERT SQL ni CSS de portada nuevos. La cuadrícula se ajustó a 16×12 celdas de 50px (800×600, el tamaño fijo de canvas que usa `GamePlayer.tsx`) en vez de 16×14 de 40px; la zona de río usa 5 carriles y la de carretera 4 (en vez de 6/5) para caber en las 12 filas. El resto de la mecánica (salto discreto de 120ms, troncos/tortugas con inmersión, 5 bocas, temporizador de ronda, 3 vidas, puntuación) sigue la spec tal cual.
